...
@section('content')
    <div class="flex flex-col flex-1 overflow-hidden">
        ...
        <div class="flex-1 p-6 bg-gray-100 overflow-y-auto">
            <h1 class="text-3xl font-base text-slate-700 mb-4">List Category</h1>
            <a href="{{ url('admin/category/create') }}" class="border bg-indigo-600 text-white px-3 py-2 rounded-full"><i
                    class="fas fa-plus mr-1"></i>Tambah</a>
            @if (session('message'))
                <p class="w-full border p-3 bg-green-300 rounded-md mt-4 -mb-2">{{ session('message') }}</p>
            @endif
            <div class="overflow-x-auto">
                <table id="category-table" class="min-w-full divide-y-2 mt-6 divide-gray-200 bg-white text-sm">
                    <thead>
                        <tr>
                            <th class="whitespace-nowrap px-4 py-2 font-medium text-gray-900">No</th>
                            <th class="whitespace-nowrap px-4 py-2 font-medium text-gray-900">Name Category</th>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @foreach ($categories as $category)
                            <tr>
                                ...
                                <td class="whitespace-nowrap px-4 py-2 text-gray-700">{{ $category->name_category }}</td>
